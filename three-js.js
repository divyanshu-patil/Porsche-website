// tell renderer to use physically correct lights
// adding environment maps
// there are two ways
// 1. using a cube map
// 2. using a HDRI (RGBELoader)
// 1. RGBELoader
// import rgbeloader
// same process as gltf loader
// 2. cubetextureloader
// updating material by scene.traverse()
// traversing all material and checking if it's mesh and have mesh standard material
// if yes then the envMap property is set to environment map
// make a debug UI tweak for controlling intensity
// set the onChange property to call updateAllMaterials
// change renderer outputEncoding
// change renderer tonemapping mainly set to ACESFilmic
// for multi sampling enable antialias to true
// casting shadow is in update all material folder
...
directionalLight.position.set(1, 1, 0.23);
...
spotlight.position.set(0.36, 2.32, -4.44);
...
rgbeLoader.load(
    ...
    (environmentMap) => {
        ...
        gltfLoader.load(
            './assets/models/gt3rs_2/porsche gt3rs.gltf',
            (gltf) => {
                gltf.scene.position.y = -0.5;
                scene.add(gltf.scene);
                gui.add(gltf.scene.rotation, 'x', Math.PI / 2, Math.PI * 2, 0.001).name('ObjRotateX');
                gui.add(gltf.scene.rotation, 'y', Math.PI / 2, Math.PI * 2, 0.001).name('ObjRotateY');
                gui.add(gltf.scene.rotation, 'z', Math.PI / 2, Math.PI * 2, 0.001).name('ObjRotateZ');
        ...
        camera.position.set(0.648, 0.141, 0.521);
        scene.add(camera);
